...
# =============================================================================
import os
os.chdir("C:\Work\Bjornafjorden\Flytebru\Dynamic_Buckling\My_Abaqus_Projects\Wind Field Script") # current directory

# =============================================================================
# Generating Node Coordinates. First, with Origin at circle center (0):
# =============================================================================
num_nodes = 49 # number of g_nodes where wind is generated
zbridge = 16
...
T = 3600 # sec. Time series duration. # best if multiple of block_duration. Good if 3600 sec
sample_freq = 4 # Hz # Good if 4 Hz
...
#    return ln2,
#    return ln3,
#plt.title('Wind Speed across the bridge g_nodes')
...
plt.scatter(u_1_freq, u_1_ps/((Iu[node_tested]*V[node_tested])**2)*u_1_freq, color='blue', label='measured u', alpha=0.6, s =2)
plt.scatter(v_1_freq, v_1_ps/((Iv[node_tested]*V[node_tested])**2)*v_1_freq, color='orange', label='measured v', alpha=0.6, s =2)
plt.scatter(w_1_freq, w_1_ps/((Iw[node_tested]*V[node_tested])**2)*w_1_freq, color='green', label='measured w', alpha=0.6, s =2)
plt.xscale('log')
plt.xlabel('freq [Hz]')
...
n_coh = 10 # number of g_nodes to assess coherence. n_coh <= num_nodes
...
for node_1 in range(n_coh):
    for node_2 in range(n_coh):
        ...
        if node_2 > node_1:
            ...
            plt.scatter(coh_freq_u, coh_u, alpha=0.6, s=4, color='blue', label='measured u')
            ...
            plt.scatter(coh_freq_v, coh_v, alpha=0.6, s=4, color='orange', label='measured v')
            ...
            plt.scatter(coh_freq_w, coh_w, alpha=0.6, s=4, color='green', label='measured w')
            plt.xlabel('freq [Hz]')
...
plt.suptitle('Top: Correlation coefficients \n Bottom: Corr. coeff. difference between expected and measured')
for i in range(3):
    ax = fig.add_subplot(2,3,i+1, projection='3d')
    ax = fig.gca(projection='3d')
    ax.set_title(str(['u','v','w'][i]), fontweight='bold')
    surf1 = ax.plot_surface(X,Y, corrcoef[i], linewidth=0.2, antialiased=True, color='orange', alpha=0.9, label='measured')
    ...
    ax.text(num_nodes, 0, 0.15, "measured", color='orange')
